...
func TestClaudeDirectAppliesCurrentInputFile(t *testing.T) {
	...
	if ds.uploads[0].Filename != "chat_log.txt" {
	...
	if !strings.Contains(prompt, "chat_log.txt 里是之前的对话记录。接续回答最后一条消息。") {
	...
	if len(full.Messages) != 1 || !strings.Contains(full.Messages[0].Content, "chat_log.txt 里是之前的对话记录。接续回答最后一条消息。") {
...
func TestClaudeCurrentInputFileUploadsToolsSeparately(t *testing.T) {
	...
	if ds.uploads[0].Filename != "chat_log.txt" || ds.uploads[1].Filename != "notes.txt" {
	...
	prompt, _ := ds.payload["prompt"].(string)
	if !strings.Contains(prompt, "notes.txt") || !strings.Contains(prompt, "TOOL CALL FORMAT") {
