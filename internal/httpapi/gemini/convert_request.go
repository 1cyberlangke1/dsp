...
func normalizeGeminiRequest(store ConfigReader, routeModel string, req map[string]any, stream bool) (promptcompat.StandardRequest, error) {
	...
	resolvedModel, err := config.ResolveModelOrError(store, requestedModel)
	if err != nil {
	...
	finalPrompt, toolNames := promptcompat.BuildOpenAIPromptForAdapter(messagesRaw, promptToolsRaw, "", thinkingEnabled)
