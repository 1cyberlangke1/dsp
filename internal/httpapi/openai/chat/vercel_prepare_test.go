...
func TestHandleVercelStreamSwitchReuploadsCurrentInputFile(t *testing.T) {
	...
	stdReq := promptcompat.StandardRequest{
		...
		FinalPrompt:             "chat_log.txt 里是之前的对话记录。接续回答最后一条消息。 工具用法写在 notes.txt 里，按那里的格式调用。",
		PromptTokenText:         "# chat_log.txt\n\n=== 1. USER ===\nhello\n\n# notes.txt\nAvailable tool descriptions and parameter schemas for this request.\n\nYou have access to these tools:\n\nTool: search\nDescription: search docs\nParameters: {\"type\":\"object\"}\n",
		HistoryText:             "# chat_log.txt\n\n=== 1. USER ===\nhello\n",
	...
	promptText, _ := payload["prompt"].(string)
	if !strings.Contains(promptText, "notes.txt") {
