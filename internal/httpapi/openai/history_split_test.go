...
func TestBuildOpenAICurrentInputContextTranscriptUsesNumberedHistorySections(t *testing.T) {
	...
	if !strings.Contains(transcript, "# chat_log.txt") {
...
func TestApplyCurrentInputFileUploadsFirstTurnWithNumberedHistoryTranscript(t *testing.T) {
	...
	if upload.Filename != "chat_log.txt" {
	...
	if !strings.Contains(out.FinalPrompt, "chat_log.txt 里是之前的对话记录。接续回答最后一条消息。") {
	...
	if !strings.Contains(out.PromptTokenText, "# chat_log.txt") || !strings.Contains(out.PromptTokenText, "=== 1. USER ===") {
...
func TestApplyCurrentInputFilePreservesFullContextPromptForTokenCounting(t *testing.T) {
	...
	if !strings.Contains(out.PromptTokenText, "# chat_log.txt") || !strings.Contains(out.PromptTokenText, "=== 1. SYSTEM ===") {
		t.Fatalf("expected prompt token text to include numbered history transcript, got %q", out.PromptTokenText)
	}
	if !strings.Contains(out.PromptTokenText, "chat_log.txt 里是之前的对话记录。接续回答最后一条消息。") {
...
func TestApplyCurrentInputFileUploadsFullContextFile(t *testing.T) {
	...
	upload := ds.uploadCalls[0]
	if upload.Filename != "chat_log.txt" {
		t.Fatalf("expected chat_log.txt upload, got %q", upload.Filename)
	}
	if upload.ModelType != "vision" {
		t.Fatalf("expected vision model type for vision request, got %q", upload.ModelType)
	}
	uploadedText := string(upload.Data)
	for _, want := range []string{"# chat_log.txt", "=== 1. SYSTEM ===", "=== 2. USER ===", "=== 3. ASSISTANT ===", "=== 4. TOOL ===", "=== 5. USER ===", "system instructions", "first user turn", "hidden reasoning", "tool result", "latest user turn", promptcompat.ThinkingInjectionMarker} {
		if !strings.Contains(uploadedText, want) {
...
func TestApplyCurrentInputFileUploadsToolsContextSeparately(t *testing.T) {
	...
	if ds.uploadCalls[0].Filename != "chat_log.txt" {
		t.Fatalf("expected first upload to be chat_log.txt, got %q", ds.uploadCalls[0].Filename)
	}
	if ds.uploadCalls[1].Filename != "notes.txt" {
		t.Fatalf("expected second upload to be notes.txt, got %q", ds.uploadCalls[1].Filename)
	}
	...
	for _, want := range []string{"# notes.txt", "Tool: search", "Description: search docs", `Parameters: {"type":"object"}`} {
	...
	if !strings.Contains(out.FinalPrompt, "chat_log.txt 里是之前的对话记录。接续回答最后一条消息。") || !strings.Contains(out.FinalPrompt, "notes.txt") {
	...
	if !strings.Contains(out.PromptTokenText, "# chat_log.txt") || !strings.Contains(out.PromptTokenText, "# notes.txt") || !strings.Contains(out.PromptTokenText, "Description: search docs") {
...
func TestApplyCurrentInputFileCarriesHistoryText(t *testing.T) {
	...
	if !strings.Contains(out.HistoryText, "# chat_log.txt") || !strings.Contains(out.HistoryText, "=== 1. SYSTEM ===") {
...
func TestChatCompletionsCurrentInputFileUploadsContextAndKeepsNeutralPrompt(t *testing.T) {
	...
	if !strings.Contains(historyText, "# chat_log.txt") || !strings.Contains(historyText, "=== 1. SYSTEM ===") {
	...
	if !strings.Contains(promptText, "chat_log.txt 里是之前的对话记录。接续回答最后一条消息。") {
...
func TestResponsesCurrentInputFileUploadsToolsSeparately(t *testing.T) {
	...
	if ds.uploadCalls[0].Filename != "chat_log.txt" || ds.uploadCalls[1].Filename != "notes.txt" {
	...
	toolsText := string(ds.uploadCalls[1].Data)
	if !strings.Contains(toolsText, "# notes.txt") || !strings.Contains(toolsText, "Tool: search") || !strings.Contains(toolsText, "Description: search docs") {
		t.Fatalf("expected tools transcript to include schema, got %q", toolsText)
	}
	promptText, _ := ds.completionReq["prompt"].(string)
	if !strings.Contains(promptText, "notes.txt") || !strings.Contains(promptText, "TOOL CALL FORMAT") {
...
func TestCurrentInputFileWorksAcrossAutoDeleteModes(t *testing.T) {
	for _, mode := range []string{"none", "single", "all"} {
		t.Run(mode, func(t *testing.T) {
			...
			promptText, _ := ds.completionReq["prompt"].(string)
			if !strings.Contains(promptText, "chat_log.txt 里是之前的对话记录。接续回答最后一条消息。") || strings.Contains(promptText, "first user turn") || strings.Contains(promptText, "latest user turn") {
