...
func TestBuildOpenAICurrentInputContextTranscriptUsesNumberedHistorySections(t *testing.T) {
	...
	if !strings.Contains(transcript, "# "+promptcompat.CurrentInputContextFilename) {
...
func TestApplyCurrentInputFileUploadsFirstTurnWithNumberedHistoryTranscript(t *testing.T) {
	...
	if !strings.Contains(out.PromptTokenText, "# "+promptcompat.CurrentInputContextFilename) || !strings.Contains(out.PromptTokenText, "=== 1. USER ===") {
...
func TestApplyCurrentInputFileUploadsFullContextFile(t *testing.T) {
	...
	upload := ds.uploadCalls[0]
	if upload.Filename != promptcompat.CurrentInputContextFilename {
		t.Fatalf("expected upload, got %q", upload.Filename)
	}
...
func TestApplyCurrentInputFileUploadsToolsContextSeparately(t *testing.T) {
	...
	for _, want := range []string{"# " + promptcompat.CurrentToolsContextFilename, "Tool: search", "Description: search docs", `Parameters: {"type":"object"}`} {
	...
	if !strings.Contains(out.PromptTokenText, "# "+promptcompat.CurrentInputContextFilename) || !strings.Contains(out.PromptTokenText, "# "+promptcompat.CurrentToolsContextFilename) || !strings.Contains(out.PromptTokenText, "Description: search docs") {
...
func TestApplyCurrentInputFileCarriesHistoryText(t *testing.T) {
	...
	if !strings.Contains(out.HistoryText, "# "+promptcompat.CurrentInputContextFilename) || !strings.Contains(out.HistoryText, "=== 1. SYSTEM ===") {
...
func TestChatCompletionsCurrentInputFileUploadsContextAndKeepsNeutralPrompt(t *testing.T) {
	...
	if !strings.Contains(historyText, "# "+promptcompat.CurrentInputContextFilename) || !strings.Contains(historyText, "=== 1. SYSTEM ===") {
